namespace ImageManager.Common.Helpers
{
    ...
    {
        /// <summary>
        /// Registers AutoMapper profiles by scanning all assemblies whose names start with "ImageManager.".
        /// </summary>
        /// <param name="services">The service collection to which the AutoMapper profiles will be added.</param>
        public static void Register(IServiceCollection services)
        {
            // Retrieve all assemblies in the current application domain that have a full name starting with "ImageManager."
            var assemblies = AppDomain.CurrentDomain.GetAssemblies()
                .Where(s => s.FullName != null && s.FullName.StartsWith("ImageManager."));
